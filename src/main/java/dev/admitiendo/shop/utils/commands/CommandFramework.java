package dev.admitiendo.shop.utils.commands;
        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (!command.permission().equals("") && !sender.hasPermission(command.permission())) {
                    sender.sendMessage(ChatColor.WHITE + "This command was not found.");
                    return true;
                }
                if (command.inGameOnly() && !(sender instanceof Player)) {
                    sender.sendMessage(CC.translate("&cThis command is inGameOnly!"));
                    return true;
                }
                try {
